...
@section('page_title', 'E-Faturalarım')

@section('panel_content')
<div class="col-md-12 border-left-3 main-content">
    <div class="title-area mb-4">
        <h5 class="title"><i class="fa fa-file-invoice"></i> {{ __('messages.my_einvoices') }}</h5>
        <div class="pull-right">
            <strong><a href="{{ route('hesabim') }}">{{ __('messages.my_account') }}</a></strong> /
            <span>{{ __('messages.my_einvoices') }}</span>
        </div>
    </div>
    ...
    <table id="datatable" class="table table-bordered table-striped">
        ...
        @empty
            <tr><td colspan="5" class="text-center p-4">{{ __('messages.no_einvoices') }}</td></tr>
        @endforelse
        </tbody>
    </table>
