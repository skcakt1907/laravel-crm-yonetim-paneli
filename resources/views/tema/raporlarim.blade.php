...
@section('page_title', 'Raporlarım')

@section('panel_content')
<div class="col-md-12 border-left-3 main-content">
    <div class="title-area mb-4">
        <h5 class="title"><i class="fa fa-chart-line"></i> {{ __('messages.my_reports') }}</h5>
    ...
    <table id="datatable" class="table table-bordered table-striped">
                ...
                <td class="text-center align-middle">
                    @if(!empty($k->dosya))
                        <div class="btn-group" role="group">
                            <a href="{{ route('rapor.indir', $k->id) }}" class="btn btn-outline-primary btn-sm" title="{{ __('messages.download_file') }}">
                                <i class="fa fa-download"></i>
                            </a>
                            <a href="{{ route('rapor.detay', $k->id) }}" class="btn btn-outline-secondary btn-sm" title="{{ __('messages.view_details_title') }}">
                                <i class="fa fa-eye"></i>
                            </a>
                        </div>
                    @else
        ...
        @empty
            <tr><td colspan="5" class="text-center p-4">{{ __('messages.no_reports') }}</td></tr>
        @endforelse
        </tbody>
    </table>
