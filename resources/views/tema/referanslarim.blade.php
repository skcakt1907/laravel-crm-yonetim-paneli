...
@section('page_title', 'Referanslarım')

@section('panel_content')
<div class="col-md-12 border-left-3 main-content">
    <div class="title-area mb-4">
        <h5 class="title"><i class="fa fa-star"></i> {{ __('messages.my_referrals') }}</h5>
        <div class="pull-right">
            <strong><a href="{{ route('hesabim') }}">{{ __('messages.my_account') }}</a></strong> /
            <span>{{ __('messages.my_referrals') }}</span>
        </div>
    </div>
    ...
    <table id="datatable" class="table table-bordered table-striped">
        ...
        @empty
            <tr><td colspan="5" class="text-center p-4">{{ __('messages.no_references_yet') }}</td></tr>
        @endforelse
        </tbody>
    </table>
