...
<title>Not Defteri — İş Ortağım Admin</title>
...
<div><h1 class="font-display text-3xl font-extrabold">📓 Not Defteri <span class="badge badge-yellow text-base">{{ count($notlar ?? []) }}</span></h1><p class="text-white/60 mt-1">Tüm notların kart şeklinde listeli</p></div>
<button type="button" onclick="openNotModal()" class="btn-y">➕ Yeni Not</button>
</div>

<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
@forelse($notlar ?? [] as $not)
<div class="stat-card glass rounded-2xl p-5 flex flex-col">
<div class="flex items-start justify-between gap-2 mb-3">
<h3 class="font-bold text-lg flex-1">📌 {{ $not->baslik }}</h3>
<div class="flex gap-1">
<button type="button" class="action-btn" title="Düzenle" onclick='openNotModal({{ json_encode(["id"=>$not->id,"baslik"=>$not->baslik,"icerik"=>$not->desc]) }})'>✏️</button>
<form action="{{ route('admin.not-defteri.sil', $not->id) }}" method="POST" onsubmit="return confirm('Bu notu silmek istediğinden emin misin?');" class="inline">@csrf @method('DELETE')<button type="submit" class="action-btn" title="Sil">🗑️</button></form>
</div>
</div>
<div class="text-sm text-white/70 whitespace-pre-wrap flex-1 max-h-48 overflow-auto">{{ $not->desc }}</div>
</div>
@empty
<div class="col-span-full glass rounded-2xl p-12 text-center text-white/50">📭 Henüz not yok — sağ üstten "Yeni Not" ekle</div>
@endforelse
</div>

<div id="notModal" class="modal-bg" onclick="if(event.target===this)closeNotModal()">
<div class="modal p-6">
<div class="flex items-center justify-between mb-4">
<h2 class="font-display text-2xl font-bold" id="notModalTitle">➕ Yeni Not</h2>
<button type="button" onclick="closeNotModal()" class="action-btn text-xl">✖</button>
</div>
<form id="notForm" action="{{ route('admin.not-defteri.ekle') }}" method="POST" class="space-y-4">@csrf
<div><label class="field-label">📛 Başlık <span class="required">*</span></label><input type="text" name="baslik" id="notBaslik" required></div>
<div><label class="field-label">📝 İçerik <span class="required">*</span></label><textarea name="icerik" id="notIcerik" rows="10" required></textarea></div>
<div class="flex justify-end gap-2 pt-2"><button type="button" onclick="closeNotModal()" class="btn-o">❌ İptal</button><button type="submit" class="btn-y">💾 Kaydet</button></div>
</form>
</div>
</div>
...
const _NOT_EKLE=@json(route('admin.not-defteri.ekle'));
const _NOT_GUNCELLE=@json(url('admin/not-defteri'));
function openNotModal(data){
  const form=document.getElementById('notForm');
  const title=document.getElementById('notModalTitle');
  document.getElementById('notBaslik').value=data?.baslik||'';
  document.getElementById('notIcerik').value=data?.icerik||'';
  if(data&&data.id){
    title.textContent='✏️ Notu Düzenle';
    form.action=_NOT_GUNCELLE+'/'+data.id+'/guncelle';
  }else{
    title.textContent='➕ Yeni Not';
    form.action=_NOT_EKLE;
  }
  document.getElementById('notModal').classList.add('show');
}
function closeNotModal(){document.getElementById('notModal').classList.remove('show');}
