...
@section('title', 'Hosting Paketi Düzenle')
...
@php
    // Eski tablo (hostingler) ile yeni tablo (hosting_paketler) field mapping
    $diskAlani  = $paket->disk_alani  ?? $paket->whm_alan          ?? '';
    $trafik     = $paket->trafik      ?? $paket->whm_atrafik       ?? '';
    $veritabani = $paket->veritabani  ?? $paket->whm_max_veritabani ?? '';
    $emailSayi  = $paket->email       ?? $paket->whm_max_email     ?? '';
    $fiyat      = $paket->fiyat       ?? $paket->tutar             ?? 0;
@endphp
...
<div class="breadcrumb">
    ...
    <span class="sep">/</span>
    <span class="current">{{ \Illuminate\Support\Str::limit($paket->adi ?? '—', 40) }}</span>
</div>
...
<form action="{{ route('admin.hosting.paketler.duzenlePost', $paket->id) }}" method="POST">
    @csrf
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <div class="spec-input">
                            ...
                            <input type="text" name="disk_alani"
                                   value="{{ old('disk_alani', $diskAlani) }}"
                                   required class="form-input">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="spec-input">
                            ...
                            <input type="text" name="trafik"
                                   value="{{ old('trafik', $trafik) }}"
                                   required class="form-input">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="spec-input">
                            ...
                            <input type="text" name="veritabani"
                                   value="{{ old('veritabani', $veritabani) }}"
                                   required class="form-input">
                        </div>
                    </div>
                    <div class="form-group">
            ...
            <div class="section" style="margin-top:16px">
                ...
                <div class="form-grid">
                    <div class="form-group full">
                        ...
                        <textarea name="aciklama" rows="3" class="form-textarea">{{ old('aciklama', $paket->aciklama ?? '') }}</textarea>
                    </div>
                    <div class="form-group full">
                        <label class="form-label">Özellikler</label>
                        <textarea name="ozellikler" rows="6" class="form-textarea"
                                  placeholder="✓ Ücretsiz SSL&#10;✓ Günlük Yedekleme">{{ old('ozellikler', $paket->ozellikler ?? '') }}</textarea>
                ...
                <div class="info-grid">
                    ...
                    @if(!empty($tableName))
                        <div class="info-item">
                            ...
                            <span class="val" style="font-family:monospace;font-size:11.5px">{{ $tableName }}</span>
                        </div>
                    @endif
                </div>
            </div>
