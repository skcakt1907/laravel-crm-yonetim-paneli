...
<title>Ürünler — İş Ortağım Admin</title>
...
<div>
<div class="flex items-center gap-2 text-sm text-white/60 mb-2"><a href="{{ route('admin.dashboard') }}" class="hover:text-yellow-400">🏠</a> <span>›</span> <span>Ürünler</span></div>
<h1 class="font-display text-3xl font-extrabold flex items-center gap-3">📦 Ürünler</h1>
</div>
<a href="{{ route('admin.urunler.ekle') }}" class="btn-y">➕ Yeni</a>
</div>
<div class="glass rounded-2xl overflow-hidden">
<table class="w-full">
<thead><tr class="text-xs uppercase border-b border-yellow-500/10" style="background: rgba(184,182,46,0.05);">
<th class="text-left font-semibold px-6 py-4 text-yellow-400">Adı</th>
...
<tbody class="text-sm">
@forelse($urunler ?? [] as $item)
<tr class="data-row border-b border-yellow-500/5">
<td class="px-6 py-4 font-semibold">{{ $item->adi ?? $item->ad ?? $item->baslik ?? $item->kod ?? '—' }}</td>
<td class="px-6 py-4">@if(($item->durum ?? 0) == 1)<span class="badge badge-success">✅ Aktif</span>@else<span class="badge badge-secondary">⏸️ Pasif</span>@endif</td>
<td class="px-6 py-4 text-right">
<a href="{{ route('admin.urunler.duzenle', $item->id) }}" class="action-btn">✏️</a>
<form action="{{ route('admin.urunler.sil', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Sil?');">@csrf @method('DELETE')<button class="action-btn">🗑️</button></form>
</td>
</tr>
@empty
<tr><td colspan="3" class="px-6 py-12 text-center text-white/50">📭 Henüz kayıt yok</td></tr>
@endforelse
</tbody>
</table>
@if($urunler->hasPages())
<div class="px-6 py-4 border-t border-yellow-500/10">
