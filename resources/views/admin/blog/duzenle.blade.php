...
<div class="breadcrumb">
    ...
    <span class="sep">/</span>
    <span class="current">{{ \Illuminate\Support\Str::limit($blog->baslik ?? $blog->adi ?? 'Düzenle', 40) }}</span>
</div>
...
<div class="page-header">
    <div>
        ...
        <div class="page-subtitle">{{ $blog->baslik ?? $blog->adi ?? '—' }}</div>
    </div>
            ...
            <div class="section">
                ...
                <div class="lang-pane active" data-pane="tr">
                    <div class="form-group">
                        ...
                        <input type="text" name="baslik" value="{{ old('baslik', $blog->baslik ?? $blog->adi ?? '') }}"
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="kisa" rows="2" class="form-textarea"
                                  placeholder="Yazının özet metni (liste sayfasında görünür)">{{ old('kisa', $blog->kisa ?? $blog->ozet ?? '') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">İçerik <span class="required">*</span></label>
                        <textarea name="icerik" class="rich-full">{{ old('icerik', $blog->icerik ?? $blog->aciklama ?? '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="lang-pane" data-pane="en">
                    <div class="form-group">
                        ...
                        <input type="text" name="adi_en" value="{{ old('adi_en', $blog->baslik_en ?? $blog->adi_en ?? '') }}" class="form-input">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Short summary (EN)</label>
                        <textarea name="ozet_en" rows="2" class="form-textarea">{{ old('ozet_en', $blog->kisa_en ?? $blog->ozet_en ?? '') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Content (EN)</label>
                        <textarea name="icerik_en" class="rich-full">{{ old('icerik_en', $blog->icerik_en ?? $blog->aciklama_en ?? '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="lang-pane" data-pane="ar" dir="rtl">
                    <div class="form-group">
                        ...
                        <input type="text" name="adi_ar" value="{{ old('adi_ar', $blog->baslik_ar ?? $blog->adi_ar ?? '') }}" class="form-input" dir="rtl">
                    </div>
                    <div class="form-group">
                        <label class="form-label">ملخص قصير (AR)</label>
                        <textarea name="ozet_ar" rows="2" class="form-textarea" dir="rtl">{{ old('ozet_ar', $blog->kisa_ar ?? $blog->ozet_ar ?? '') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">المحتوى (AR)</label>
                        <textarea name="icerik_ar" class="rich-full">{{ old('icerik_ar', $blog->icerik_ar ?? $blog->aciklama_ar ?? '') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="section" style="margin-top:16px">
                ...
                <div class="form-grid">
                    <div class="form-group full">
                        ...
                        <input type="text" name="seo" value="{{ old('seo', $blog->seo ?? '') }}"
                               class="form-input"
                    ...
                    <div class="form-group full">
                        ...
                        <input type="text" name="keywords" value="{{ old('keywords', $blog->keywords ?? $blog->seo_anahtar ?? '') }}"
                               class="form-input"
                    ...
                    <div class="form-group full">
                        ...
                        <textarea name="description" rows="2" maxlength="160"
                                  ...
                                  placeholder="Arama motorlarında görünen açıklama (max 160 karakter)">{{ old('description', $blog->description ?? $blog->seo_aciklama ?? '') }}</textarea>
            ...
            <div class="section">
                ...
                @if(!empty($blog->resim))
                    <div style="margin-bottom:10px">
                        <img src="{{ asset('tema/uploads/bloglar/' . $blog->resim) }}"
                             alt="Mevcut görsel"
                ...
                <div class="image-upload" onclick="document.getElementById('resimInput').click()">
                    ...
                    <div style="font-weight:600;margin-top:6px">{{ !empty($blog->resim) ? 'Görseli değiştir' : 'Görsel yükle' }}</div>
                    ...
                    <select name="dil" class="form-select">
                        @if(isset($diller) && count($diller) > 0)
                            @foreach($diller as $d)
                                ...
                                @endphp
                                <option value="{{ $dId }}" {{ old('dil', (int)($blog->dil ?? 1)) == $dId ? 'selected' : '' }}>
                                    {{ $dAd }}
                                </option>
                            @endforeach
                        @else
                            <option value="1" {{ old('dil', (int)($blog->dil ?? 1)) == 1 ? 'selected' : '' }}>🇹🇷 Türkçe</option>
                            <option value="2" {{ old('dil', (int)($blog->dil ?? 1)) == 2 ? 'selected' : '' }}>🇬🇧 English</option>
                            <option value="3" {{ old('dil', (int)($blog->dil ?? 1)) == 3 ? 'selected' : '' }}>🇸🇦 العربية</option>
                        @endif
                    </select>
                ...
                <div class="toggle-card">
                    ...
                    <label class="ios-toggle">
                        <input type="checkbox" name="durum" value="1"
                               {{ old('durum', (int)($blog->durum ?? 1)) ? 'checked' : '' }}>
...
{{-- SİL FORMU --}}
<form id="del-blog-form"
      action="{{ route('admin.blog.sil', $blog->id) }}"
...
<script>
function silBlog() {
    if (confirm('Bu blog yazısını silmek istediğinize emin misiniz?\n\n{{ addslashes($blog->baslik ?? $blog->adi ?? '') }}\n\nBu işlem geri alınamaz.')) {
        document.getElementById('del-blog-form').submit();
    }
}
