...
<title>Promo Onay — İş Ortağım Admin</title>
...
<div class="mb-6">
<div class="flex items-center gap-2 text-sm text-white/60 mb-2"><a href="{{ route('admin.dashboard') }}" class="hover:text-yellow-400">🏠</a> <span>›</span> <span>Promo Kodlar</span></div>
<h1 class="font-display text-3xl font-extrabold flex items-center gap-3">🎟️ Promo Kod Onayları <span class="badge badge-warning text-base">{{ count($promolar ?? []) }}</span></h1>
<p class="text-white/60 mt-1">Bayilerin oluşturduğu promo kodları onayla/reddet</p>
</div>

<div class="glass rounded-2xl overflow-hidden">
<table class="w-full">
<thead><tr class="text-xs uppercase border-b border-yellow-500/10" style="background: rgba(184,182,46,0.05);">
<th class="text-left font-semibold px-6 py-4 text-yellow-400">Kod</th>
<th class="text-left font-semibold px-6 py-4 text-yellow-400">Bayi</th>
<th class="text-left font-semibold px-6 py-4 text-yellow-400">İndirim</th>
<th class="text-left font-semibold px-6 py-4 text-yellow-400">Limit</th>
<th class="text-left font-semibold px-6 py-4 text-yellow-400">Bitiş</th>
<th class="text-left font-semibold px-6 py-4 text-yellow-400">Durum</th>
<th class="text-right font-semibold px-6 py-4 text-yellow-400">İşlem</th>
</tr></thead>
<tbody class="text-sm">
@forelse($promolar ?? [] as $p)
<tr class="data-row border-b border-yellow-500/5">
<td class="px-6 py-4"><code class="bg-yellow-500/10 text-yellow-400 px-2 py-1 rounded text-base">{{ $p->kod }}</code></td>
<td class="px-6 py-4">{{ $p->firmaadi ?? $p->bayi_kodu ?? '—' }}</td>
<td class="px-6 py-4 font-bold">@if($p->indirim_tipi=='yuzde')%{{ $p->indirim_miktari }}@else₺{{ $p->indirim_miktari }}@endif</td>
<td class="px-6 py-4">{{ $p->kullanim_limiti ?? '∞' }}</td>
<td class="px-6 py-4 text-xs text-white/60">{{ $p->bitis_tarihi ?? '—' }}</td>
<td class="px-6 py-4">@if($p->onay_durumu == 1)<span class="badge badge-success">✅ Onaylı</span>
@elseif($p->onay_durumu == 2)<span class="badge badge-danger">❌ Red</span>
@else<span class="badge badge-warning">⏳ Bekliyor</span>@endif</td>
<td class="px-6 py-4 text-right">
@if($p->onay_durumu == 0)
<form action="{{ route('admin.promo.onayla', $p->id) }}" method="POST" class="inline">@csrf<button class="action-btn" title="Onayla">✅</button></form>
<form action="{{ route('admin.promo.reddet', $p->id) }}" method="POST" class="inline">@csrf<button class="action-btn" title="Reddet">❌</button></form>
@endif
</td>
</tr>
@empty
<tr><td colspan="7" class="px-6 py-12 text-center text-white/50">🎉 Onay bekleyen promo kod yok</td></tr>
@endforelse
</tbody>
</table>
</div>
