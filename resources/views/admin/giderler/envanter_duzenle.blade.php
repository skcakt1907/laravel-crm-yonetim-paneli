...
@section('title', 'Envanter Düzenle')
...
<div class="gfo-head"><h1><i data-lucide="pencil"></i> Envanter Düzenle</h1></div>
...
<form action="{{ route('admin.giderler.envanter.guncelle', $kalem->id) }}" method="POST" class="gfo-card">
    @csrf
    @method('PUT')
    <div class="gfo-row tek">
        <div class="gfo-grup">
            <label class="gfo-label">Ürün / Malzeme Adı <span class="req">*</span></label>
            <input type="text" name="ad" class="gfo-input" value="{{ old('ad', $kalem->ad) }}" required autofocus>
        </div>
    </div>
    ...
    <div class="gfo-row">
        <div class="gfo-grup">
            ...
            <input type="text" name="kategori" class="gfo-input" value="{{ old('kategori', $kalem->kategori) }}" placeholder="Örn: Bilgisayar, Mobilya...">
        </div>
        <div class="gfo-grup">
            <label class="gfo-label">Durum <span class="req">*</span></label>
            <select name="durum" class="gfo-select" required>
                <option value="kullanimda" {{ old('durum', $kalem->durum)==='kullanimda'?'selected':'' }}>Kullanımda</option>
                <option value="depoda" {{ old('durum', $kalem->durum)==='depoda'?'selected':'' }}>Depoda</option>
                <option value="arizali" {{ old('durum', $kalem->durum)==='arizali'?'selected':'' }}>Arızalı</option>
                <option value="elden_cikti" {{ old('durum', $kalem->durum)==='elden_cikti'?'selected':'' }}>Elden Çıktı</option>
            </select>
        </div>
    </div>
    ...
    <div class="gfo-row">
        <div class="gfo-grup">
            ...
            <input type="number" name="adet" class="gfo-input" min="1" value="{{ old('adet', $kalem->adet) }}" required>
        </div>
        <div class="gfo-grup">
            <label class="gfo-label">Birim Fiyat <span class="req">*</span></label>
            <div class="gfo-fiyat-wrap">
                <span class="sym">₺</span>
                <input type="number" step="0.01" min="0" name="birim_fiyat" class="gfo-input" value="{{ old('birim_fiyat', $kalem->birim_fiyat) }}" required>
            </div>
        </div>
    </div>
    ...
    <div class="gfo-row tek">
        <div class="gfo-grup">
            ...
            <input type="date" name="alis_tarihi" class="gfo-input" value="{{ old('alis_tarihi', $kalem->alis_tarihi) }}">
        </div>
    </div>
    ...
    <div class="gfo-row tek">
        <div class="gfo-grup">
            ...
            <textarea name="aciklama" class="gfo-textarea">{{ old('aciklama', $kalem->aciklama) }}</textarea>
        </div>
    </div>
