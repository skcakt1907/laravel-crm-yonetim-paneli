...
<div class="gfo-head"><h1><i data-lucide="plus-circle"></i> Yeni Gider</h1></div>
...
<form action="{{ route('admin.giderler.store') }}" method="POST" class="gfo-card">
    @csrf

    <div class="gfo-row tek">
        <div class="gfo-grup">
            <label class="gfo-label">Başlık <span class="req">*</span></label>
            <input type="text" name="baslik" class="gfo-input" value="{{ old('baslik') }}" required placeholder="Örn: Ofis kirası, Adobe aboneliği..." autofocus>
        </div>
    </div>
    ...
    <div class="gfo-row">
        <div class="gfo-grup">
            ...
            <select name="kategori_id" class="gfo-select">
                ...
                <option value="{{ $k->id }}" {{ (string)old('kategori_id')===(string)$k->id?'selected':'' }}>{{ $k->ad }}</option>
                @endforeach
            </select>
        </div>
        <div class="gfo-grup">
    ...
    <div class="gfo-row">
        <div class="gfo-grup">
            ...
            <input type="date" name="gider_tarihi" class="gfo-input" value="{{ old('gider_tarihi', date('Y-m-d')) }}">
        </div>
        <div class="gfo-grup">
            <label class="gfo-label">Son Ödeme Tarihi</label>
            <input type="date" name="son_odeme_tarihi" class="gfo-input" value="{{ old('son_odeme_tarihi') }}">
        </div>
    </div>
    ...
    <div class="gfo-row">
        <div class="gfo-grup">
            ...
            <select name="durum" class="gfo-select" required>
                <option value="bekliyor" {{ old('durum','bekliyor')==='bekliyor'?'selected':'' }}>Bekliyor</option>
                <option value="odendi" {{ old('durum')==='odendi'?'selected':'' }}>Ödendi</option>
                <option value="gecikti" {{ old('durum')==='gecikti'?'selected':'' }}>Gecikti</option>
            </select>
        </div>
        <div class="gfo-grup">
            ...
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:9px 0;margin:0">
                <input type="checkbox" name="tekrar_eden" value="1" id="tekrarEden" {{ old('tekrar_eden')?'checked':'' }} onchange="document.getElementById('tekrarAyWrap').style.display=this.checked?'block':'none'">
                <span style="font-size:13px">Bu gider periyodik tekrarlasın (kira, maaş, abonelik vb.)</span>
            </label>
        </div>
        <div class="gfo-grup" id="tekrarAyWrap" style="display:{{ old('tekrar_eden')?'block':'none' }}">
            <label class="gfo-label">Kaç ayda bir?</label>
            <input type="number" name="tekrar_ay" class="gfo-input" min="1" max="60" value="{{ old('tekrar_ay', 1) }}" placeholder="1">
    ...
    <div class="gfo-row tek">
        <div class="gfo-grup">
            ...
            <input type="text" name="odeme_yontemi" class="gfo-input" value="{{ old('odeme_yontemi') }}" placeholder="Örn: Kredi kartı, Havale, Nakit...">
        </div>
    </div>
    ...
    <button type="submit" class="gfo-submit"><i data-lucide="check" style="width:18px;height:18px"></i> Gideri Kaydet</button>
</form>
@endsection
