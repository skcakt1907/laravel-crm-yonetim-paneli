...
@section('title', 'Aylık Ödeme Düzenle')
...
<div class="gfo-head"><h1><i data-lucide="pencil"></i> Aylık Ödeme Düzenle</h1></div>
...
@php $secili = (int) old('periyot_ay', $odeme->periyot_ay); @endphp

<form action="{{ route('admin.aylik-odemeler.guncelle', $odeme->id) }}" method="POST" class="gfo-card">
    @csrf @method('PUT')

    <div class="gfo-row tek">
        <div class="gfo-grup">
            <label class="gfo-label">Başlık <span class="req">*</span></label>
            <input type="text" name="baslik" class="gfo-input" value="{{ old('baslik', $odeme->baslik) }}" required>
        </div>
    </div>
    ...
    <div class="gfo-row">
        <div class="gfo-grup">
            ...
            <select name="kategori_id" class="gfo-select kategori-select">
                ...
                <option value="{{ $k->id }}" data-diger="{{ mb_strtolower($k->ad,'UTF-8')==='diğer'?1:0 }}" {{ (string)old('kategori_id',$odeme->kategori_id)===(string)$k->id?'selected':'' }}>{{ $k->ad }}</option>
                @endforeach
            </select>
            <input type="text" name="kategori_diger" class="gfo-input kategori-diger" style="margin-top:8px;display:none" placeholder="Diğer — ne olduğunu yazın…" value="{{ old('kategori_diger', $odeme->kategori_diger ?? '') }}" maxlength="150">
        </div>
        <div class="gfo-grup">
            <label class="gfo-label">Tutar <span class="req">*</span></label>
            <div class="gfo-tutar-wrap">
                <span class="sym">₺</span>
                <input type="number" step="0.01" min="0" name="tutar" class="gfo-input" value="{{ old('tutar', $odeme->tutar) }}" required>
            </div>
        </div>
    </div>
    ...
    <div class="gfo-row">
        ...
        <div class="gfo-grup">
            ...
            <input type="date" name="son_odeme_tarihi" class="gfo-input" value="{{ old('son_odeme_tarihi', $odeme->son_odeme_tarihi) }}" required>
        </div>
    </div>
    ...
    <div class="gfo-row">
        <div class="gfo-grup">
            ...
            <select name="durum" class="gfo-select" required>
                <option value="bekliyor" {{ old('durum',$odeme->durum)==='bekliyor'?'selected':'' }}>Ödenmedi (Bekliyor)</option>
                <option value="odendi" {{ old('durum',$odeme->durum)==='odendi'?'selected':'' }}>Ödendi</option>
            </select>
        </div>
        <div class="gfo-grup">
            <label class="gfo-label">Ödeme Yöntemi</label>
            <input type="text" name="odeme_yontemi" class="gfo-input" value="{{ old('odeme_yontemi', $odeme->odeme_yontemi) }}" placeholder="Örn: Havale, Kredi kartı...">
        </div>
    </div>
    ...
    <div class="gfo-row tek">
        <div class="gfo-grup">
            ...
            <textarea name="aciklama" class="gfo-textarea">{{ old('aciklama', $odeme->aciklama) }}</textarea>
        </div>
    </div>

    <button type="submit" class="gfo-submit"><i data-lucide="check" style="width:18px;height:18px"></i> Güncelle</button>
</form>
@endsection
